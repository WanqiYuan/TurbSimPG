...
namespace Seb.Fluid.Rendering
{
	...
	public class ParticleDisplay3D : MonoBehaviour
	{
		...
		[Tooltip("If true, map temperature to color; if false, map velocity to color")]
		public bool useTemperatureMapping = false;
		[Tooltip("Maximum temperature value for color mapping")]
		public float temperatureDisplayMax = 100f;
		...
		void UpdateSettings()
		{
			if (modeOld != mode)
			{
				...
				if (mode != DisplayMode.None)
				{
					...
					mat.SetBuffer("Temperature", sim.temperatureBuffer);
				}
			}
			...
			if (mat != null)
			{
				...
				mat.SetFloat("temperatureMax", temperatureDisplayMax);
				mat.SetInt("useTemperatureMapping", useTemperatureMapping ? 1 : 0);
